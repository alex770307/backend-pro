package lesson_01.appStructureVar2.back.repository;
...
//        return foundedTasks;
    ...
    public Optional<Task> updateById(Integer taskId, Task updatedTask) {
        Optional<Task> optionalTask = findById(taskId);
        if (optionalTask.isPresent()) {
            Task taskForUpdate = optionalTask.get();
            // Обновляем поля задачи
            taskForUpdate.setName(updatedTask.getName());
            taskForUpdate.setDescription(updatedTask.getDescription());
            // Возвращаем обновленную задачу
            return Optional.of(taskForUpdate);
        }
        return Optional.empty(); // Если задача не найдена, возвращаем пустой Optional
    }
